...
import { productInput, ProductNotFoundError, updateProductInput } from "../types";
...
export default class ProductWriter {
  ...
  try {
    ...
    static async increaaseProductQuantity(productId: string, increment: number): Promise<void> {
        const updatedProduct = await IproductModel.findByIdAndUpdate(
            productId,
